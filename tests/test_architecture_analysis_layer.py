...
def test_architecture_layer_computes_fan_metrics_and_transitive_dependents(tmp_path: Path) -> None:
    ...
    vectors = ArchitectureAnalysisLayer().run(_vectors(tmp_path, consumer, service, util))
    by_path = {vector.relative_path: vector for vector in vectors}

    consumer_vector = by_path["src/pkg/consumer.py"]
    service_vector = by_path["src/pkg/service.py"]
    util_vector = by_path["src/pkg/util.py"]
...
def test_architecture_layer_reports_scc_metadata_and_circular_dependency_size(tmp_path: Path) -> None:
    ...
    vectors = ArchitectureAnalysisLayer().run(_vectors(tmp_path, a, b, c))
...
def test_architecture_layer_ignores_external_imports(tmp_path: Path) -> None:
    ...
    vector = ArchitectureAnalysisLayer().run(_vectors(tmp_path, module))[0]
...
def _vectors(root: Path, *paths: Path) -> list[MetricsVector]:
    return [
        MetricsVector(
            layer=ArchitectureAnalysisLayer.LAYER_NAME,
            absolute_path=path,
