...
def test_decision_layer_scores_files_between_zero_and_one_and_ranks_pressure() -> None:
    ...
    file_decision_vectors = [
        layer.run([vector for vector in vectors if vector.file_path == "/src/hot.py"]),
        layer.run([vector for vector in vectors if vector.file_path == "/src/calm.py"]),
    ]
    decision_vectors = [*file_decision_vectors, layer.summarize(file_decision_vectors)]
    by_file = {
        vector.file_path: vector
        for vector in decision_vectors
        if vector.file_path is not None
    }
    summary = next(vector for vector in decision_vectors if vector.file_path is None)
